    ...
    const fetchSchedules = useCallback(async () => {
        try {
            ...
            if (filters?.teacherId) {
                mappedData = mappedData.filter((s: any) => s.teacher_id === filters.teacherId);
            }
            if (filters?.sectionId) {
                mappedData = mappedData.filter((s: any) => s.section_id === filters.sectionId);
            }
            if (filters?.dayOfWeek) {
                mappedData = mappedData.filter((s: any) => s.day_of_week === filters.dayOfWeek);
            }
            if (filters?.status) {
    ...
    const fetchStats = useCallback(async () => {
        ...
        const [teachers, rooms, schedulesRes] = await Promise.all([
            ...
            supabase.from('schedules')
                .select('*, subject:subjects(*), teacher:teachers(*, profile:profiles!teachers_profile_id_fkey(*)), room:rooms(*), section:sections(*)')
