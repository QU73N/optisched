        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                {upcomingEvents.length > 0 && (
                    <View style={{ marginBottom: 16 }}>
                        <Text style={{ fontSize: 11, fontWeight: '600', color: '#A9B4C2', letterSpacing: 1.5, marginBottom: 8 }}>UPCOMING EVENTS</Text>
                ...
                {showQuickActions && (
                    <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
                        {[
                            { icon: 'calendar-month', color: '#4988C4', bg: 'rgba(59,130,246,0.12)', label: 'Schedule', screen: 'tasks' },
                            ...
                            { icon: 'warning', color: '#E6A23C', bg: 'rgba(245,158,11,0.12)', label: 'Alerts', screen: 'alerts' },
                        ].map((item, i) => (
                    ...
                    {announcements.length === 0 ? (
                        ...
                        announcements.slice(0, 5).map(ann => (
                            <View key={ann.id} style={styles.announcementCard}>
                                <View style={[styles.annDot, { backgroundColor: priorityColors[ann.priority] || '#4988C4' }]} />
                                <View style={{ flex: 1 }}>
                                ...
                                <View style={styles.annActions}>
                                    ...
                                    <AnimatedPressable
                                        ...
                                        hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                                    >
                                        <MaterialIcons name="delete-outline" size={16} color="#E05D5D" />
                                    </AnimatedPressable>
                                </View>
                            </View>
                        ))
                    ...
                    <View style={styles.section}>
                        ...
                        {teacherMessages.slice(0, 5).map(msg => (
                            <AnimatedPressable key={msg.id} onPress={() => openConversation(msg)} activeOpacity={0.7}
                                style={{ backgroundColor: '#263241', borderRadius: 12, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: msg.status === 'unread' ? 'rgba(59,130,246,0.3)' : '#1E2935' }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                        {msg.status === 'unread' && (
                                            <View style={{ backgroundColor: 'rgba(59,130,246,0.2)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>
                                                <Text style={{ fontSize: 9, fontWeight: '700', color: '#4988C4' }}>NEW</Text>
                                            </View>
                                        )}
            ...
            <Modal visible={showAnnouncementModal} animationType="fade" transparent>
                <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', paddingHorizontal: 20 }}>
                    <View style={{ backgroundColor: '#263241', borderRadius: 20, overflow: 'hidden', maxHeight: '80%' }}>
                        {/* Fixed Header */}
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#1E2935' }}>
                        ...
                        <ScrollView style={{ paddingHorizontal: 20 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 40 }}>
                                <AnimatedPressable onPress={() => setAnnSection('All Sections')} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, marginRight: 8, backgroundColor: annSection === 'All Sections' ? Colors.primary : '#0B0F14', borderWidth: 1, borderColor: annSection === 'All Sections' ? Colors.primary : '#1E2935' }}>
                                    <Text style={{ color: annSection === 'All Sections' ? '#fff' : '#A9B4C2', fontSize: 12, fontWeight: '600' }}>All Sections</Text>
                                </AnimatedPressable>
                                {sections.map(s => (
                                    <AnimatedPressable key={s.id} onPress={() => setAnnSection(s.name)} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, marginRight: 8, backgroundColor: annSection === s.name ? Colors.primary : '#0B0F14', borderWidth: 1, borderColor: annSection === s.name ? Colors.primary : '#1E2935' }}>
                                        <Text style={{ color: annSection === s.name ? '#fff' : '#A9B4C2', fontSize: 12, fontWeight: '600' }}>{s.name}</Text>
                                    </AnimatedPressable>
                                ))}
                            </ScrollView>
                            <View style={{ height: 8 }} />
                        ...
                        {/* Fixed Footer Button */}
                        <View style={{ paddingHorizontal: 20, paddingVertical: 16, borderTopWidth: 1, borderTopColor: '#1E2935' }}>
                            <AnimatedPressable
            ...
            <Modal visible={showEditModal} animationType="slide" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <AnimatedPressable style={[styles.modalBtn, { backgroundColor: '#E6A23C' }]} onPress={handleUpdateAnnouncement} disabled={updating}>
            ...
            <Modal visible={quickScreen !== null} animationType="slide">
                <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0F14' }}>
                    <AnimatedPressable
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#263241' }}
            ...
            <Modal visible={showConvoModal} animationType="slide" transparent>
                <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.8)' }}>
                    <View style={{ flex: 1, marginTop: 60, backgroundColor: '#0B0F14', borderTopLeftRadius: 24, borderTopRightRadius: 24, overflow: 'hidden' }}>
                        {/* Convo Header */}
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#263241' }}>
                            ...
                            {convoMessages.map((m, i) => {
                                ...
                                return (
                                    <View key={m.id || i} style={{ alignSelf: isAdmin ? 'flex-end' : 'flex-start', maxWidth: '80%', marginBottom: 10 }}>
                                        <View style={{ backgroundColor: isAdmin ? Colors.primary : '#263241', borderRadius: 16, padding: 12, borderBottomRightRadius: isAdmin ? 4 : 16, borderBottomLeftRadius: isAdmin ? 16 : 4 }}>
                        ...
                        {/* Reply Input */}
                        <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1, borderTopColor: '#263241', backgroundColor: '#141925' }}>
                            <TextInput
                                style={{ flex: 1, backgroundColor: '#263241', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, color: Colors.white, fontSize: 14, maxHeight: 80, borderWidth: 1, borderColor: '#1E2935' }}
                                placeholder="Type a reply..." placeholderTextColor="#64748b"
                                value={replyText} onChangeText={setReplyText} multiline
                            />
                            <AnimatedPressable
                                style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: replyText.trim() ? Colors.primary : '#1E2935', justifyContent: 'center', alignItems: 'center' }}
            ...
            <Modal visible={showEventModal} animationType="slide" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <AnimatedPressable
                            style={[styles.modalBtn, { backgroundColor: '#3FAF73', marginTop: 20 }]}
                            onPress={async () => {
...
const styles = StyleSheet.create({
    ...
    heroSubtitle: { fontSize: 14, color: '#BDE8F5', marginBottom: 24 },
    ...
    heroStatLabel: { fontSize: 12, color: '#BDE8F5', marginBottom: 4 },
