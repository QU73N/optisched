...
const AdminManageUsers: React.FC = () => {
    ...
    const getRoleBadgeColor = (role: string) => {
        switch (role) {
            case 'admin': return { bg: 'rgba(239,68,68,0.15)', text: '#E05D5D' };
            case 'teacher': return { bg: 'rgba(59,130,246,0.15)', text: '#4988C4' };
                        ...
                        return (
                            ...
                            >
                                ...
                                <AnimatedPressable
                                    ...
                                    hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                                >
                                    {deleting === user.id ? <ActivityIndicator size="small" color="#E05D5D" /> : <MaterialIcons name="delete-outline" size={20} color="#E05D5D" />}
                                </AnimatedPressable>
                                                ...
                                                [...new Set(dbSections.map((s: any) => s.program).filter(Boolean))].map((prog: string) => (
                                                    <AnimatedPressable
                                                        ...
                                                        style={{
                                                            ...
                                                            borderColor: newUser.program === prog ? '#6366f1' : '#1E2935',
                                                        }}
                                                        onPress={() => setNewUser(p => ({ ...p, program: prog, yearLevel: '', section: '' }))}
                                                    >
                                                        <Text style={{ color: newUser.program === prog ? '#fff' : '#A9B4C2', fontWeight: '600', fontSize: 13 }}>{prog}</Text>
                                                    </AnimatedPressable>
                                                ))
                                            ) : (
                                                ...
                                                {[...new Set(
                                                    ...
                                                    <AnimatedPressable
                                                        ...
                                                        style={{
                                                            ...
                                                            borderWidth: 1.5,
                                                            borderColor: newUser.yearLevel === String(yr) ? '#6366f1' : '#1E2935',
                                                        }}
                                                        onPress={() => setNewUser(p => ({ ...p, yearLevel: String(yr), section: '' }))}
                                                    >
                                                        <Text style={{ color: newUser.yearLevel === String(yr) ? '#fff' : '#A9B4C2', fontWeight: '700', fontSize: 14 }}>
                                                        ...
                                                        .map((sec: any) => (
                                                            <AnimatedPressable
                                                                ...
                                                                style={{
                                                                    ...
                                                                    borderColor: newUser.section === sec.name ? '#6366f1' : '#1E2935',
                                                                }}
                                                                onPress={() => setNewUser(p => ({ ...p, section: sec.name }))}
                                                            >
                                                                <Text style={{ color: newUser.section === sec.name ? '#fff' : '#A9B4C2', fontWeight: '600', fontSize: 13 }}>{sec.name}</Text>
                                                            </AnimatedPressable>
                                                        ))}
                        ...
                        {selectedUser && (
                            <View style={styles.passwordUserInfo}>
                                <View style={styles.passwordUserAvatar}>
                                    <MaterialIcons name={selectedUser.role === 'teacher' ? 'school' : 'person'} size={28} color="#4988C4" />
                                </View>
                                <View style={{ flex: 1 }}>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.passwordModalContent}>
                        ...
                        {editingUser && (
                            <View style={styles.passwordUserInfo}>
                                <View style={styles.passwordUserAvatar}>
                                    <MaterialIcons name="person" size={28} color="#4988C4" />
                                </View>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                                    ...
                                    {dbSections && [...new Set(dbSections.map((s: any) => s.program).filter(Boolean))].map((prog: string) => (
                                        <AnimatedPressable
                                            ...
                                            style={{
                                                ...
                                                borderColor: editProgram === prog ? '#6366f1' : '#1E2935',
                                            }}
                                            onPress={() => { setEditProgram(prog); setEditYearLevel(''); setEditSection(''); }}
                                        >
                                            <Text style={{ color: editProgram === prog ? '#fff' : '#A9B4C2', fontWeight: '600', fontSize: 13 }}>{prog}</Text>
                                        </AnimatedPressable>
                                    ))}
                                </View>
                            </ScrollView>
                                        ...
                                        {[...new Set(
                                            ...
                                            <AnimatedPressable
                                                ...
                                                style={{
                                                    ...
                                                    borderWidth: 1.5, borderColor: editYearLevel === String(yr) ? '#6366f1' : '#1E2935',
                                                }}
                                                onPress={() => { setEditYearLevel(String(yr)); setEditSection(''); }}
                                            >
                                                <Text style={{ color: editYearLevel === String(yr) ? '#fff' : '#A9B4C2', fontWeight: '700', fontSize: 14 }}>
                                                    {yr >= 11 ? `Grade ${yr}` : `Year ${yr}`}
                                                </Text>
                                            </AnimatedPressable>
                                        ))}
                                            ...
                                            {dbSections?.filter((sec: any) => {
                                                ...
                                                <AnimatedPressable
                                                    ...
                                                    style={{
                                                        ...
                                                        borderColor: editSection === sec.name ? '#6366f1' : '#1E2935',
                                                    }}
                                                    onPress={() => setEditSection(sec.name)}
                                                >
                                                    <Text style={{ color: editSection === sec.name ? '#fff' : '#A9B4C2', fontWeight: '600', fontSize: 13 }}>{sec.name}</Text>
                                                </AnimatedPressable>
                                            ))}
                                        </View>
                                    </ScrollView>
                            ...
                            {/* Current values */}
                            <View style={{ backgroundColor: 'rgba(99,102,241,0.08)', borderRadius: 8, padding: 10, marginBottom: 12, borderWidth: 1, borderColor: '#1E2935' }}>
                                <Text style={{ color: '#A9B4C2', fontSize: 11, fontWeight: '600' }}>WILL SAVE:</Text>
...
const styles = StyleSheet.create({
    ...
    avatarText: { color: '#4988C4', fontWeight: '700', fontSize: 14 },
    userInfo: { flex: 1 },
    ...
    modalContent: {
        backgroundColor: '#263241', borderTopLeftRadius: 24, borderTopRightRadius: 24,
    ...
    roleOption: {
        ...
        backgroundColor: '#0B0F14', borderWidth: 1, borderColor: Colors.borderDark
    },
    ...
    passwordUserInfo: {
        flexDirection: 'row', alignItems: 'center', gap: 12,
        backgroundColor: '#0B0F14', borderRadius: 12, padding: 14,
    ...
    validationError: {
        fontSize: 12, color: '#E05D5D', marginTop: 6
    }
});
