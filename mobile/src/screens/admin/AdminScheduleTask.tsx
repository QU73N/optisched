...
const iconMap: Record<string, { iconName: string; iconBg: string; iconColor: string }> = {
    ...
    medium: { iconName: 'fact-check', iconBg: 'rgba(16,185,129,0.15)', iconColor: '#3FAF73' },
...
const AdminScheduleTask: React.FC = () => {
    ...
    const tasks = useMemo(() => {
        ...
        return filtered.map(task => {
            ...
            const badgeColor = task.priority === 'high' ? '#fb923c' : task.priority === 'medium' ? '#E6A23C' : Colors.slate400;
            return {
                ...task,
                ...icons,
                badgeLabel,
    ...
    const progressColors: Record<string, string> = { high: Colors.primary, medium: '#3FAF73', low: '#a855f7' };
    ...
    const priorityBorderColors: Record<string, string> = { high: '#E05D5D', medium: '#E6A23C', low: Colors.primary };
                        ...
                        <View style={styles.taskFooter}>
                            {task.status !== 'completed' ? (
                                <AnimatedPressable style={styles.completeSmall} onPress={() => handleMarkComplete(task.id, task.title)}>
                                    <MaterialIcons name="check" size={14} color="#3FAF73" />
                                    <Text style={{ color: '#3FAF73', fontSize: 11, fontWeight: '600' }}>Complete</Text>
                                </AnimatedPressable>
                            ) : (
                                <View style={[styles.completeSmall, { backgroundColor: 'rgba(16,185,129,0.2)' }]}>
                                    <MaterialIcons name="check-circle" size={14} color="#3FAF73" />
                                    <Text style={{ color: '#3FAF73', fontSize: 11, fontWeight: '600' }}>Done</Text>
                                </View>
                            )}
                            ...
                            {(['high', 'medium', 'low'] as const).map(p => (
                                <AnimatedPressable key={p} onPress={() => setModalPriority(p)}
                                    style={{
                                        ...
                                        borderColor: modalPriority === p ? priorityBorderColors[p] : '#1E2935',
...
const styles = StyleSheet.create({
    ...
    modalContent: { backgroundColor: '#263241', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 24, paddingTop: 24, paddingBottom: 40 },
    ...
    modalInput: { backgroundColor: '#0B0F14', borderWidth: 1, borderColor: '#1E2935', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, color: Colors.white, fontSize: 14 },
