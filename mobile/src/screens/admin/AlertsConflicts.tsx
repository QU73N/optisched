...
const AlertsConflicts: React.FC = () => {
    ...
    const priorityConfig: Record<string, { color: string; bg: string; label: string }> = {
        high: { color: '#E05D5D', bg: 'rgba(239,68,68,0.15)', label: 'HIGH PRIORITY' },
        medium: { color: '#E6A23C', bg: 'rgba(245,158,11,0.15)', label: 'MEDIUM PRIORITY' },
        low: { color: '#A9B4C2', bg: 'rgba(148,163,184,0.15)', label: 'LOW PRIORITY' }
    };

    const typeIcons: Record<string, { icon: string; color: string }> = {
        room: { icon: 'meeting-room', color: '#E05D5D' },
        teacher: { icon: 'directions-run', color: '#E6A23C' },
        unassigned: { icon: 'domain-disabled', color: '#A9B4C2' },
        capacity: { icon: 'groups', color: '#A9B4C2' }
    };
                            ...
                            <View style={styles.alertMeta}>
                                ...
                                {alert.source === 'detected' && (
                                    <View style={{ backgroundColor: 'rgba(16,185,129,0.15)', paddingHorizontal: 6, paddingVertical: 1, borderRadius: 4 }}>
                                        <Text style={{ color: '#3FAF73', fontSize: 9, fontWeight: '600' }}>LIVE</Text>
                                    </View>
                                )}
                            </View>
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        {warningCount > 0 && (
                            <>
                                <View style={[styles.sectionHeader, { marginTop: 8 }]}>
                                    <MaterialIcons name="warning" size={24} color="#E6A23C" />
                        ...
                        {filteredAlerts.length === 0 && (
                            <View style={{ alignItems: 'center', paddingVertical: 60 }}>
                                <MaterialIcons name="check-circle" size={56} color="#3FAF73" />
                                <Text style={{ color: '#3FAF73', fontSize: 18, fontWeight: '600', marginTop: 16 }}>All Clear!</Text>
            ...
            <Modal visible={showResolutionModal} animationType="slide" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        {resolutionDetails && (
                            <View>
                                <View style={{ backgroundColor: '#0B0F14', borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#1E2935' }}>
                                    <Text style={{ color: '#E05D5D', fontSize: 11, fontWeight: '600', letterSpacing: 1, marginBottom: 8 }}>CONFLICT</Text>
                                ...
                                <View style={{ backgroundColor: 'rgba(16,185,129,0.08)', borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: 'rgba(16,185,129,0.2)' }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                                        <MaterialIcons name="lightbulb" size={16} color="#3FAF73" />
                                        <Text style={{ color: '#3FAF73', fontSize: 11, fontWeight: '600', letterSpacing: 1 }}>SUGGESTED FIX</Text>
                                    </View>
                                ...
                                {resolutionDetails.action === 'reassign_room' ? (
                                    ...
                                    <AnimatedPressable
                                        style={[styles.applyBtn, { backgroundColor: '#E6A23C' }]}
                                        onPress={() => {
                                            applyResolution();
                                        }}
                                    >
...
const styles = StyleSheet.create({
    ...
    applyBtn: {
        backgroundColor: '#3FAF73', borderRadius: 12, paddingVertical: 16,
