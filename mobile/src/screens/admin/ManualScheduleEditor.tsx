...
const ManualScheduleEditor: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        const [schedRes, teacherRes, roomRes, subjectRes, sectionRes] = await Promise.all([
            ...
            supabase.from('teachers').select('*, profile:profiles!teachers_profile_id_fkey(*)'),
        ...
        <ScrollView style={{ flex: 1, backgroundColor: '#0B0F14' }} contentContainerStyle={{ padding: 16 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                {teachers.map(t => (
                    ...
                    >
                        <MaterialIcons name="person" size={14} color={selectedTeacher === t.id ? Colors.white : '#A9B4C2'} />
                            ...
                            <View key={sch.id} style={s.schedCard}>
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', gap: 12, marginTop: 4 }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                                            <MaterialIcons name="meeting-room" size={12} color="#A9B4C2" />
                                            <Text style={s.schedMeta} numberOfLines={1}>{sch.room_name}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                                            <MaterialIcons name="groups" size={12} color="#A9B4C2" />
                                ...
                                <View style={{ flexDirection: 'row', gap: 6 }}>
                                    <AnimatedPressable style={s.iconBtn} onPress={() => openEdit(sch)}>
                                        <MaterialIcons name="edit" size={16} color="#4988C4" />
                                    </AnimatedPressable>
                                    <AnimatedPressable style={s.iconBtn} onPress={() => handleDelete(sch)}>
                                        <MaterialIcons name="delete" size={16} color="#E05D5D" />
                                    </AnimatedPressable>
                                </View>
                            </View>
                        ))}
            ...
            <Modal visible={showAddModal} animationType="slide" transparent>
                <View style={s.modalOverlay}>
                    <View style={s.modalContent}>
                        ...
                        <View style={{ paddingHorizontal: 20, paddingBottom: 16 }}>
                            <AnimatedPressable style={[s.saveBtn, { backgroundColor: '#3FAF73' }]} onPress={handleAdd} disabled={adding}>
...
const s = StyleSheet.create({
    ...
    teacherChip: {
        ...
        backgroundColor: '#263241', borderWidth: 1, borderColor: '#1E2935', marginRight: 8,
    },
    teacherChipActive: { backgroundColor: Colors.primary, borderColor: Colors.primary },
    teacherChipText: { fontSize: 13, fontWeight: '600', color: '#A9B4C2' },
    addBtn: {
    ...
    emptyState: { alignItems: 'center', paddingVertical: 40, backgroundColor: '#263241', borderRadius: 16, marginBottom: 16 },
    dayHeader: { fontSize: 14, fontWeight: '700', color: '#818cf8', marginBottom: 8, letterSpacing: 0.5 },
    schedCard: {
        flexDirection: 'row', alignItems: 'center', gap: 12,
        backgroundColor: '#263241', borderRadius: 14, padding: 14, marginBottom: 8,
    ...
    schedMeta: { fontSize: 11, color: '#A9B4C2' },
    iconBtn: {
        width: 32, height: 32, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.05)',
        justifyContent: 'center', alignItems: 'center',
    },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' },
    modalContent: { backgroundColor: '#263241', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '80%' },
    modalHeader: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
        paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#1E2935',
    },
    ...
    input: {
        backgroundColor: '#0B0F14', borderRadius: 10, padding: 12, color: Colors.white,
        fontSize: 14, borderWidth: 1, borderColor: '#1E2935', marginBottom: 12,
    },
    dayChip: {
        paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
        backgroundColor: '#0B0F14', borderWidth: 1, borderColor: '#1E2935', marginRight: 8,
    },
    dayChipActive: { backgroundColor: Colors.primary, borderColor: Colors.primary },
    dayChipText: { fontSize: 12, fontWeight: '600', color: '#A9B4C2' },
    saveBtn: {
