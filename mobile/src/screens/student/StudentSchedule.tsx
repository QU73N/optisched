...
const COLORS = [
    ...
    { bg: 'rgba(16,185,129,0.18)', border: '#3FAF73', text: '#6ee7b7', solid: '#3FAF73' },
...
const StudentSchedule: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('student_schedule_realtime')
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border }]}>
                <View style={styles.headerTop}>
                    <View>
                        ...
                        <Text style={[styles.headerSub, { color: colors.textSecondary }]}>{currentMonth}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <AnimatedPressable onPress={() => setWeekOffset(w => w - 1)} style={{ padding: 6, backgroundColor: colors.isDark ? 'rgba(73,136,196,0.15)' : 'rgba(28,77,141,0.08)', borderRadius: 8 }}>
                            <MaterialIcons name="chevron-left" size={20} color={colors.accentPrimary} />
                        </AnimatedPressable>
                        {weekOffset !== 0 && (
                            <AnimatedPressable onPress={() => setWeekOffset(0)} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: colors.isDark ? 'rgba(73,136,196,0.15)' : 'rgba(28,77,141,0.08)', borderRadius: 8 }}>
                                <Text style={{ color: colors.accentPrimary, fontSize: 12, fontWeight: '600' }}>Today</Text>
                            </AnimatedPressable>
                        )}
                        <AnimatedPressable onPress={() => setWeekOffset(w => w + 1)} style={{ padding: 6, backgroundColor: colors.isDark ? 'rgba(73,136,196,0.15)' : 'rgba(28,77,141,0.08)', borderRadius: 8 }}>
                            <MaterialIcons name="chevron-right" size={20} color={colors.accentPrimary} />
                        </AnimatedPressable>
                    </View>
                    <View style={[styles.classCountBadge, { backgroundColor: colors.isDark ? 'rgba(73,136,196,0.12)' : 'rgba(28,77,141,0.08)', borderColor: colors.isDark ? 'rgba(73,136,196,0.2)' : 'rgba(28,77,141,0.15)' }]}>
                        <Text style={[styles.classCountNum, { color: colors.accentPrimary }]}>{scheduleItems.length}</Text>
                        <Text style={[styles.classCountLabel, { color: colors.accentPrimary }]}>classes</Text>
                    </View>
                </View>
                ...
                <View style={styles.calendarStrip}>
                    {daysOfWeek.map((day, index) => {
                        ...
                        return (
                            <AnimatedPressable
                                key={day}
                                style={[styles.calDay, isActive && [styles.calDayActive, { backgroundColor: colors.isDark ? 'rgba(73,136,196,0.12)' : 'rgba(28,77,141,0.08)' }]]}
                                onPress={() => setSelectedDay(index)}
                            >
                                <Text style={[styles.calDayLabel, { color: colors.textMuted }, isActive && { color: colors.accentPrimaryHover }]}>{day}</Text>
                                <View style={[styles.calDateCircle, isActive && styles.calDateCircleActive]}>
                                    <Text style={[styles.calDate, { color: colors.textSecondary }, isActive && styles.calDateActive]}>{weekDates[index]}</Text>
                                </View>
            ...
            {nextClass && (
                <View style={styles.nextClassBanner}>
                    ...
                    <Text style={[styles.nextClassName, { color: colors.textPrimary }]} numberOfLines={1}>{nextClass.subject}</Text>
                                ...
                                >
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={[styles.blockCode, { color: colors.textMuted }]} numberOfLines={1}>{item.code}</Text>
                                        </View>
                                        {totalCols <= 2 && (
                                    ...
                                    {height > 75 && (
                                        <View style={[styles.meta, { marginTop: 2 }]}><MaterialIcons name="person" size={10} color={colors.textMuted} /><Text style={[styles.metaText, { color: colors.textMuted }]} numberOfLines={1}>{item.instructor}</Text></View>
                                    )}
                                </AnimatedPressable>
                            );
                        })}
...
const styles = StyleSheet.create({
    ...
    classCountNum: { fontSize: 20, fontWeight: '800' },
    classCountLabel: { fontSize: 10, fontWeight: '600' },
    ...
    calDayActive: {},
    calDayLabel: { fontSize: 11, fontWeight: '600', marginBottom: 6 },
    calDayLabelActive: {},
    calDateCircle: {
    ...
    calDateActive: { color: '#ffffff' },
    todayMarker: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#4988C4', marginTop: 4 },

    nextClassBanner: {
        flexDirection: 'row', alignItems: 'center', gap: 8,
        marginHorizontal: 20, marginTop: 8, marginBottom: 4,
        backgroundColor: 'rgba(63,175,115,0.1)', borderRadius: 12,
        paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: 'rgba(63,175,115,0.2)'
    },
    nextClassDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#3FAF73' },
    nextClassLabel: { fontSize: 12, color: '#34d399', fontWeight: '600' },
    nextClassName: { flex: 1, fontSize: 13, fontWeight: '600' },
