...
function useFetch<T>(
    ...
    // Separate effect for subscriptions - doesn't depend on fetchData callback
    useEffect(() => {
        // Subscribe to real-time changes on this table
        const channelId = `realtime_${tableName}_${Math.random().toString(36).slice(2, 9)}`;
...
export function useSchedules(filters?: {
    ...
    *,
    ...
    // Separate effect for realtime subscription — doesn't depend on fetchSchedules callback
    useEffect(() => {
        // Realtime subscription for schedule changes
        const channelId = `schedules_realtime_${Math.random().toString(36).slice(2, 9)}`;
        const channel = supabase
            .channel(channelId)
...
export function useAdminDashboardStats() {
    ...
    useEffect(() => {
        ...
        // Real-time updates for stats — use a ref to avoid re-subscribing on fetchStats changes
        const tables = ['schedules', 'conflicts', 'teachers', 'rooms'];
        const channels = tables.map(table => {
            const channelId = `stats_${table}_${Math.random().toString(36).slice(2, 9)}`;
            return supabase
                .channel(channelId)
...
export function useAnnouncements() {
    ...
    // Update ref whenever fetchAnnouncements changes
    const fetchRef = useRef(fetchAnnouncements);
    useEffect(() => {
        fetchRef.current = fetchAnnouncements;
    }, [fetchAnnouncements]);

    // Real-time subscription for announcements — separate effect, doesn't depend on fetchAnnouncements callback
    useEffect(() => {
        // Realtime subscription for announcements
        const channelId = `announcements_realtime_${Math.random().toString(36).slice(2, 9)}`;
        const channel = supabase
            .channel(channelId)
...
export function useScheduleChangeRequests(filterStatus?: string) {
    ...
    useEffect(() => {
        const channelName = `sched_req_rt_${filterStatus || 'all'}_${Math.random().toString(36).slice(2, 9)}`;
        const channel = supabase
            .channel(channelName)
