    ...
    try {
        switch (action) {
            ...
            case 'update_profile': {
                ...
                return { success: true, message: `Profile updated for ${user_email || user_id}.${updates.email ? ` Email changed to ${updates.email} — user can now log in with the new email.` : ''}` };
            }
